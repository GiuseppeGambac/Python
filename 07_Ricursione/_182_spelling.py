def check_spell (stringa, lista):
    parolafinale =""
   
    if len(stringa) > 1:
     if not stringa[0] in lista:
        
        if stringa[0:1] in lista:
            parolafinale += lista[stringa[0:1]]
            parolafinale += (check_spell(stringa[2:],lista))
            return parolafinale
     else:
            parolafinale += lista[stringa[0]]
            return (check_spell(stringa[1],lista))
...
print(check_spell("silver", elements))